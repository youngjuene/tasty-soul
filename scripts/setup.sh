...
#
# CI 가 `shellcheck -S warning` 으로 이 파일을 검사한다. 고치기 전에 로컬에서도 돌리려면:
#   brew install shellcheck && shellcheck -S warning scripts/setup.sh
#
# ─────────────────────────────────────────────────────────────────────────────
...
# cd 가 실패하면 **즉시 멈춘다.** 그러지 않으면 이후 단계가 엉뚱한 디렉토리에서
# `.env` 를 쓰고 `npm install` 을 돌린다 (SC2164).
cd "$(dirname "$0")/.." || { echo "저장소 루트로 이동하지 못했습니다" >&2; exit 1; }
