 ...
 * 화면 5 — 대시보드 (§13 화면 5).
 *
 * 패널 2개. **모든 데이터는 §12에서 이미 정의된 값만 쓴다.**
 * 이 화면은 순수 뷰다 — 축 값을 계산하거나 셀을 판정하지 않는다 (§2 · §12.1).
 *
 *   1. 2×2 셀 분포     `cells` / `cells_recent30`
 *   2. 최근 이동 축    `|axes_change[a]|` 상위 3개
 *
 * §13 화면 5 는 패널 셋을 적어 두었고 그중 하나가 `timeline` 의
 * `(drift, crystal)` 연결 산점도였다. **뺐다.** 시간 축이 없는 평면 위의 경로라
 * 읽으려면 아래에 시간 스트립을 따로 깔아야 했고, 그 둘을 겹쳐 읽는 것이 이 앱에서
 * 가장 어려운 일이었다. 대시보드는 한눈에 읽히는 자리로 두고, 흩어진 것을 훑는 일은
 * 아카이브가 맡는다.
 *
 * 다만 아카이브가 그 패널을 대신하지는 못한다 — 아카이브는 **항목**을 감각 축에
 * 흩뿌리고, 그 패널은 **프로필 자체**가 달마다 움직인 자취였다. 지금 그 자취는
 * 어디에도 없다. 머리글의 `또렷함` 한 숫자와 `SOUL.md` 의 월별 표가 남은 흔적이다.
 *
 * `prompt_sha256` 경계(§R11)도 시간 축 위의 세로 구분선을 잃었다. 경고 문장 자체는
 * 패널 2에 그대로 남아 있다 — 그 문장은 지우지 않는다 (`ui/README.md` 7 — 프라이버시 문구).
 *
...
import type { CellName, Derived } from "../lib/types";
...
export default function Dashboard({ onOpenArchive }: DashboardProps = {}) {
  ...
  // 패널 2 — |axes_change[a]| 상위 3개. 값은 커맨드가 줬다.
    ...
    <div className="dash">
      <header className="dash-head">
        ...
        <dl className="dash-meta">
          ...
          {/*
            `crystal` 이 화면에 남은 유일한 자리다. `SOUL.md` 는 §8.2 템플릿이라
            같은 값을 "해상도"로 적으므로, 두 이름은 `TERMS.crystal.doc` 이 이어 준다.
          */}
          <div>
            <dt>또렷함</dt>
      ...
      <div className="dash-grid">
        ...
        <section className="panel">
          ...
          {/*
            §R11 — 이 문장은 줄이지 않는다 (`ui/README.md` 7 — 프라이버시 문구). 다만 "프롬프트 경계"가
