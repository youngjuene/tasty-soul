...
export function BoundaryNotice({
  ...
  return (
    <section className="doc doc-boundary" aria-labelledby="bn-title">
      ...
      {/*
        §D2 표는 한 행도 빼지 않는다 (§D7 · `ui/README.md` 7 — 프라이버시 문구). 그런데 이 화면은
        **앱에서 가장 먼저 뜨는 화면**이고, 표만 놓으면 처음 온 사람은 15줄짜리
        표를 만나 읽지 않고 확인 버튼을 누른다. 읽히지 않은 고지는 고지가 아니다.
        그래서 표를 줄이는 대신 **표를 세 문장으로 먼저 요약해 준다.** 요약이
        표를 대신하지 않도록 바로 아래에 표가 그대로 이어진다.
      */}
      <div className="doc-tldr">
        <p>
          <strong>넣은 것은 밖으로 나갑니다.</strong> 사진·소리·글은 그것이 무엇인지 읽히기 위해
          OpenAI 로 전송됩니다. 이것 없이는 앱이 동작하지 않습니다.
        </p>
        <p>
          <strong>무언가 넣을 때마다 웹 검색이 일어납니다.</strong> 검색어에는 지금 무엇에 관심이
          있는지가 그대로 드러나고, 그것이 검색 회사로 갑니다. 아래에서 끄고 시작할 수 있습니다.
        </p>
        <p>
          <strong>직접 쓴 문장을 담는 자리 하나는 절대 나가지 않습니다</strong> —{" "}
          <code className="doc-code">SOUL.md</code> 의{" "}
          <code className="doc-code">soul:human</code> 부분. 원본 파일도 복사되지 않습니다.
        </p>
        <p className="doc-muted">
          위 세 줄은 요약입니다. 정확한 전부는 아래 표에 한 줄도 빼지 않고 적혀 있습니다.
        </p>
      </div>
